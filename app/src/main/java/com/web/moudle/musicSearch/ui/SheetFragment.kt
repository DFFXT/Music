...
class SheetFragment : BaseSearchFragment() {
    ...
    override fun viewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.rvMusicList.setItemDecoration(
            DrawableItemDecoration(
                bottom = 20, left = 20, right = 20,
                drawable = ResUtil.getDrawable(R.drawable.dash_line_1px), orientation = RecyclerView.VERTICAL
            )
        )
        adapter = SimpleSheetAdapter()
        adapter.itemClick = {
            if (it != null) { // 进入歌单
                SongSheetActivity.actionStart(requireContext(), it.sheetId)
            }
        }
