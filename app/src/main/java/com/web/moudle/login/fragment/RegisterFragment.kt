...
import com.music.m.databinding.FragmentRegisterBinding
...
class RegisterFragment : BaseFragment<FragmentRegisterBinding>() {
    ...
    override fun initView(rootView: View) {
        binding.tvRegister.setOnClickListener {
            ...
            val pwd1 = binding.etPassword.text.toString()
            val pwd2 = binding.etPasswordConfirm.text.toString()
    ...
    private fun check(account: String, pwd1: String, pwd2: String, nikcName: String): Boolean {
        if (account.length < 4 || account.length > 11) {
            MToast.showToast(requireContext(), "--------?")
            return false
        }

        if (pwd1 != pwd2) {
            MToast.showToast(requireContext(), "pwd1!=pwd2")
            return false
        }
