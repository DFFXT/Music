...
object ResUtil {
    ...
    @JvmStatic
    fun getSize(@DimenRes id: Int, ctx: Context? = MyApplication.context): Int {
        return (ctx ?: MyApplication.context).resources.getDimensionPixelSize(id)
    }
    ...
    fun getSpannable(
        ...
        while (index >= 0) {
            spannable.setSpan(
                TextAppearanceSpan("", 0, mSize, ColorStateList.valueOf(color), null),
                index,
                index + render.length,
    ...
    @JvmStatic
    fun getColor(@ColorRes colorId: Int, ctx: Context? = MyApplication.context): Int {
        return (ctx ?: MyApplication.context).resources.getColor(colorId)
    }
    ...
    @JvmStatic
    fun getDrawable( @DrawableRes drawableId: Int, ctx: Context = MyApplication.context): Drawable {
        val theme = try {
            ctx.theme
        } catch (e: Exception) {
            null
    ...
    @JvmStatic
    fun getBitmapFromResoucs(@DrawableRes drawableId: Int, ctx: Context = MyApplication.context): Bitmap {
        return getBitmapFromDrawable(getDrawable(drawableId, ctx))
    }
